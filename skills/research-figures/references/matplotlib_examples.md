...
> **How this file fits the skill.** This is a general plot-type *cookbook* — line/bar/scatter/box/violin/heatmap, multi-panel layout, significance bars, seaborn and plotly — to draw on when the field-specific catalog in `figure_recipes.md` does not already cover your plot type. The examples below were written against a standalone `save_publication_figure(...)` helper, **not** the skill's canonical export API. For real submissions, swap that helper for `figure_export.save_figure(fig, name, journal=..., width=...)` (see "Helper Function for Saving" below) so sizing/DPI come from `journal_guidelines.md` automatically. The vector-first / reproducible-from-code / no-AI-image rule and the Okabe-Ito discipline from the main SKILL.md still apply to every example here. For fine-grained control of any single plot element or a novel plot type, see the separate **matplotlib** skill.

## Overview

This reference provides practical code examples for creating publication-ready scientific figures using Matplotlib, Seaborn, and Plotly. All examples follow best practices from `publication_guidelines.md` and use colorblind-friendly palettes from `color_palettes.md` (and the canonical `assets/palettes.py`). Seaborn and Plotly are **optional** tools for quick statistical and interactive plots respectively; the skill's default and submission path is matplotlib + numpy via `figure_export.py`.
...
**Canonical path (use this for submissions).** Apply the skill style and export with the
journal-aware helper from `scripts/figure_export.py`, which sets the correct column width
and DPI from `references/journal_guidelines.md`:

```python
import sys, pathlib
sys.path.insert(0, str(pathlib.Path("scripts").resolve()))   # research-figures/scripts on path
from figure_export import apply_style, save_figure, grayscale_preview
...
**Standalone fallback (no skill on path).** The examples below call a minimal
`save_publication_figure(...)` so each snippet is self-contained and copy-pasteable. It does
**not** set journal width/DPI — `figure_export.save_figure()` does. Treat the snippet helper as
a stand-in; replace it with `save_figure()` for anything destined for a journal.

```python
def save_publication_figure(fig, filename, formats=['pdf', 'png'], dpi=300):
    """
    Minimal standalone saver (stand-in for figure_export.save_figure).

    For real submissions prefer:
        save_figure(fig, filename, journal=..., width=...)
    which applies the correct column width + DPI automatically.
