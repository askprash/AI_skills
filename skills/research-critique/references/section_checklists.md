...
See `statistical_pitfalls.md` for deeper coverage. For trials and ML methods, apply the relevant reporting standard from `reporting_standards.md` (CONSORT/SPIRIT for trials; TRIPOD+AI / NeurIPS-style checklists for ML).
...
- For ML tools, check reproducibility against the NeurIPS / Pineau ML reproducibility checklists (see `reporting_standards.md`).
