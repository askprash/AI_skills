#!/usr/bin/env python3
"""
Figure Export Utilities for Publication-Ready Scientific Figures (ALTERNATIVE helper).

NOTE — NOT the canonical export module for this skill.
    The canonical export API for research-figures is `figure_export.py`
    (apply_style / save_figure / grayscale_preview). The five worked scripts and
    all SKILL.md examples use that module, and its journal table is derived from
    `references/journal_guidelines.md`.

    This module is kept as an *alternative / legacy* helper. Its API
    (save_publication_figure / save_for_journal / check_figure_size) is independent
    of figure_export.py and uses a different (biomedical-leaning) journal set
    (nature/science/cell/plos/acs/ieee). Use it only if you specifically need one
    of those journals or the standalone size-check / font-embedding helpers below;
    otherwise prefer figure_export.save_figure().
