...
> **Scope note.** General, field-agnostic best practices for publication figures
> (resolution, typography, color, layout, accessibility, common mistakes, a submission
> checklist). For the per-journal hard specs of this skill's target venues, see
> `journal_guidelines.md` (authoritative); for the worked field figure catalog, see
> `figure_recipes.md`. Everything here is compatible with the canonical
> `figure_export.py` workflow — these are the principles its defaults encode.
