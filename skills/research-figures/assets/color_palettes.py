"""
Colorblind-Friendly Color Palettes for Scientific Visualization (SUPPLEMENTARY).

NOTE — NOT the canonical palette module for this skill.
    The canonical palette module for research-figures is `assets/palettes.py`,
    which defines OKABE_ITO + OKABE_ITO_LIST and the WARMING_COOLING diverging
    scheme used by the worked scripts (e.g. forcing_bar_chart.py). Import from there
    for anything the worked examples rely on.

    This module is SUPPLEMENTARY: it offers extra named categorical palettes (Wong,
    Paul Tol bright/muted/light/high-contrast) and convenience helpers
    (apply_palette / get_palette). Use it when you need a palette beyond Okabe-Ito.
    Its OKABE_ITO_LIST is the same eight colors as palettes.py but in a different
    order, so do not mix the two within one figure.

This module provides carefully curated color palettes optimized for
scientific publications and accessibility.

Usage (supplementary palettes):
    from color_palettes import WONG, TOL_MUTED, apply_palette
    import matplotlib.pyplot as plt

    apply_palette('tol_muted')
