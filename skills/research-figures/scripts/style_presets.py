#!/usr/bin/env python3
"""
Matplotlib Style Presets for Publication-Ready Scientific Figures (ALTERNATIVE helper).

NOTE — NOT the canonical style module for this skill.
    The canonical styling API for research-figures is `figure_export.apply_style`,
    which loads the user's own `assets/prash_print.mplstyle` (publication) or
    `assets/prash_slides.mplstyle` (talks). That is what the five worked scripts use.

    This module is kept as an *alternative / legacy* helper that builds rcParams
    dictionaries in code (apply_publication_style / configure_for_journal /
    set_color_palette) for a biomedical-leaning journal set (nature/science/cell/...).
    It is handy for quick experiments or for the extra named palettes (Wong, Tol),
    but prefer apply_style('print') for anything destined for submission so the
    output matches the rest of the skill.
