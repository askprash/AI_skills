...
### Abstract Structure: The Seven-Move Nature Summary Paragraph

While written as flowing prose, a high-quality abstract performs seven moves in a fixed order — the structure *Nature* prescribes for its "summary paragraph," which transfers to almost any journal:

1. **Basic introduction** (1-2 sentences): Comprehensible to a scientist in *any* discipline; orient the complete outsider.
2. **Detailed background** (2-3 sentences): Comprehensible to *related* fields; narrow to the subfield and state the relevant prior consensus.
3. **The specific problem** (1 sentence): State clearly the problem this study addresses (often opens with "However,…").
4. **The main result** (1 sentence): The single central finding, signposted with "Here we show" or an equivalent.
5. **Results in context** (2-3 sentences): What the result reveals relative to what was previously thought, with concrete quantitative data.
6. **General context** (1-2 sentences): The wider frame — the take-home message.
7. **Broader perspective** (2-3 sentences, optional): Future directions or cross-disciplinary relevance, comprehensible to any scientist.

The result is an **hourglass**: broad opening → narrow problem and key result → broad implications. Target ≈190 words through move 6, ≈250 with move 7, and never exceed 300.

**For the annotated Nature worked example, a sentence-by-sentence construction guide, venue word-count targets, common failure modes, and a quality checklist, refer to `references/abstract_writing.md`.**
...
- State a concrete result, not just an activity ("Here we show that X drives Y," not "we investigate X")
- Do not cite references in the abstract (exception: *Nature*-family journals, whose summary paragraphs use numbered citations)
