...
**Abstract Composition**: The abstract is the most-read part of any paper and the primary basis for editorial and reviewer first impressions—craft it with disproportionate care. Build it as a self-contained, seven-move *Nature*-style summary paragraph: (1) a basic introduction comprehensible to any scientist, (2) more detailed background for related fields, (3) one sentence stating the specific problem, (4) one sentence stating the main result with "Here we show" or an equivalent, (5) the results in context of what was previously thought, (6) the general context, and (7) an optional broader perspective. This forms an hourglass that opens broad, narrows to a single key result, then widens to implications. Default to flowing prose: a complete summary paragraph runs ≈190 words through move 6 and ≈250 words with the optional move 7, never exceeding a 300-word ceiling, and is compressed to fit any shorter limit the journal sets. Use labeled structured abstracts only when the journal explicitly requires them. **For full sentence-by-sentence guidance, the annotated Nature example, word-count targets by venue, common failure modes, and a quality checklist, refer to `references/abstract_writing.md`.**
...
7. Craft Abstract last, synthesizing the complete story using the seven-move *Nature* summary-paragraph structure (see `references/abstract_writing.md`)
...
- `references/abstract_writing.md`: Sentence-by-sentence guide to high-impact abstracts, built on the *Nature* summary-paragraph model with an annotated example, venue word-count targets, and a quality checklist
