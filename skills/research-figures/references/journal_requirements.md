...
> **Authoritative source note.** For the journals this skill targets (the EGU / AGU /
> Nature-family / Science / AIAA / RSC set), the authoritative specs live in
> `journal_guidelines.md`, and `figure_export.save_figure()` reads its width/DPI table
> from there. This file is a broader, *general* reference covering additional publishers
> (Cell Press, PLOS, IEEE, ACS, Elsevier, BMC) for when you submit elsewhere. Where the
> two overlap, `journal_guidelines.md` wins. Always verify against the live author page.
